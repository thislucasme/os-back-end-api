...
import { FolhaPagamentoService } from 'src/folha-pagamento/folha-pagamento.service';
import { getExpenseTypeLabel } from 'src/ordens-servico/constant/expense-types';
...
export class PagamentosService {
    constructor(
        ...
        @Inject(forwardRef(() => FolhaPagamentoService)) // <-- use forwardRef para o service
    ...
    async marcarPago(user: any, payload: MarcarPagoPayloadDto): Promise<MarcarPagoResponseDto> {
        ...
        if (!dados_para_folha_pagamento.contaSaidaId) throw new Error("É necessário selecionar uma conta bancária.")

        await this.folhaPagamentoService.create(user, {
            usuarioId: dados_para_folha_pagamento.usuarioId,
            mes: dados_para_folha_pagamento.periodo.mes,
            ano: dados_para_folha_pagamento.periodo.ano,
            salarioBase: dados_para_folha_pagamento.salarioBase,
            comissaoTotal: dados_para_folha_pagamento.comissaoTotal,
            descontos: dados_para_folha_pagamento.descontos,
            totalLiquido: dados_para_folha_pagamento.totalLiquido,
        });

        const descricaoBase = 'Pagamento de folha';
        const descricao = `Folha de pagamento - ${dados_para_folha_pagamento.nome}`;

        const hoje = new Date();

        const createDto: CreateDespesaDto = {
            contaFinanceiraId: dados_para_folha_pagamento.contaSaidaId,
            descricao,
            valor: Number(dados_para_folha_pagamento.totalLiquido),
            dataDespesa: hoje.toISOString().split('T')[0],
            observacao: `Pagamento da folha do colaborador ${dados_para_folha_pagamento.nome} (ID ${dados_para_folha_pagamento.usuarioId}).`,
        };

        //await this.despesasService.create({ id: userId }, createDto);
        await this.despesasService.create({ id: dados_para_folha_pagamento.usuarioId }, createDto);
